package com.example.twister.log;

import android.app.Application;
import android.os.AsyncTask;
import android.util.Log;

import com.example.twister.util.DateUtil;
...
final public class LumberYard {
  private static final int BUFFER_SIZE = 200;

  private static  Application app;

  private static  Deque<Entry> entries = new ArrayDeque<>(BUFFER_SIZE + 1);
  private static  PublishSubject<Entry> entrySubject = PublishSubject.create();
  ...
  public static Timber.Tree tree() {
  ...
  private static synchronized void addEntry(Entry entry) {
  ...
  public static List<Entry> bufferedLogs() {
    return new ArrayList<>(entries);
  }

  public static Observable<Entry> logs() {
    return entrySubject;
  }

  /**  Save the current logs to disk. */
  public static Observable<File> save() {
    return Observable.create(new Observable.OnSubscribe<File>() {
      @Override public void call(Subscriber<? super File> subscriber) {
        ...
        String fileName = DateUtil.nowAsString() + ".log";
